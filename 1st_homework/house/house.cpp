/*
Дом. Дом имеет несколько подъездов по N этажей. В каждом подъезде на площадке распологается ровно по К квартир.
Определить по заданному номеру квартиры номер подъезда, в котором она находится и номер этажа.
*/
#include <iostream>
using namespace std;
